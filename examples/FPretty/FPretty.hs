...
normalise :: Doc -> Doc
normalise = observe "normalise" normalise'
normalise' d = td :<> sd
  where
  (td,sd) = go d Nil
...
interpret = observe "interpret" interpret'
...
outLine = observe "outLine" outLine'
...
outNest = observe "outNest" outNest'
...
outText = observe "outText" outText'
...
outAlign = observe "outAlign" outAlign'
...
extendFrontGroup = observe "extendFrontGroup" extendFrontGroup'
...
leaveGroup = observe "leaveGroup" leaveGroup'
leaveGroup' tc p ds = 
  case viewl ds of
    EmptyL -> tc p ds
    (s1,outGrp1) :< ds1 -> 
