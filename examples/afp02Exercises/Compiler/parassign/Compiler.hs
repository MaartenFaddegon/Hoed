...
compObey sm Skip = 
  []
compObey sm (xs := es) =
  let sms = iterate push sm in
  concat (zipWith compEval sms (reverse es)) ++
  concat (zipWith compStore (iterate pop (sms !! length es)) xs)
compObey sm (c1 :-> c2) =
  compObey sm c1 ++
  compObey sm c2
...
compStore :: StackMap -> Name -> [Instruction]
compStore sm x = [Store (location sm x)]
