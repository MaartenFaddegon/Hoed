...
myStackSet :: T
myStackSet = StackSet {current = Screen {workspace = Workspace {tag = NonNegative 0, layout = -2, stack = Just (Stack {focus = 'o', up = "a", down = "b"})}, screen = 0, screenDetail = 0}, visible = [], hidden = [], floating = M.fromList []}

main :: IO ()
main = runOwp propositions $ do
  g <- newStdGen
  -- print . fromJust . ok . (generate 1 g) . evaluate $  (prop_swap_all_r swapDown) myStackSet
  print (foldr (const swapDown) myStackSet [1..(length (index myStackSet))])
  where
    propositions =
