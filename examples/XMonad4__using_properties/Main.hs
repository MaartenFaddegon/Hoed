...
myStackSet :: T
myStackSet = StackSet {current = Screen {workspace = Workspace {tag = NonNegative 4, layout = 1, stack = Just (Stack {focus = 'e', up = "", down = ""})}, screen = 1, screenDetail = 1}, visible = [Screen {workspace = Workspace {tag = NonNegative 2, layout = 1, stack = Nothing}, screen = 0, screenDetail = 1}], hidden = [Workspace {tag = NonNegative 0, layout = 1, stack = Nothing},Workspace {tag = NonNegative 1, layout = 1, stack = Nothing},Workspace {tag = NonNegative 3, layout = 1, stack = Just (Stack {focus = 'w', up = "", down = ""})},Workspace {tag = NonNegative 5, layout = 1, stack = Just (Stack {focus = 'g', up = "", down = "z"})}], floating = M.fromList []}


main :: IO ()
main = runOwp propositions $ do
  g <- newStdGen
  print . fromJust . ok . (generate 1 g) . evaluate $  (prop_allWindowsMember member) 'w' myStackSet
  where
    propositions =
        ...
        , Propositions [ mkProposition module_Properties "prop_allWindowsMember"
                       ...
                       PropertiesOf "member" [module_StackSet, module_QuickCheck, module_Map, module_Map_fromList, module_Random, module_Maybe]
