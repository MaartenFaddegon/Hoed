...
prop_view_current (x :: T) (n :: NonNegative Int) = {-# SCC "prop_view_current" #-} i `tagMember` x ==>
...
mytest a n = {-# SCC "mytest" #-} mycheck defaultConfig
    { configMaxTest=n
    , configEvery   = \n args -> let s = show n in s ++ [ '\b' | _ <- s ] } a
 -- , configEvery= \n args -> if debug then show n ++ ":\n" ++ unlines args else [] } a

mycheck :: Testable a => Config -> a -> IO (Bool, Int)
mycheck config a = {-# SCC "mycheck" #-} do
    rnd <- newStdGen
