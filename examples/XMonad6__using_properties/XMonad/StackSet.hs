...
swapUp    = observe "swapUp" $ modify' swapUp'
swapDown  = observe "swapDown" $ modify' (reverseStack . swapUp' . reverseStack)
...
swapUp' :: Observable a => Stack a -> Stack a
swapUp' = observe "swapUp'" swapUp'_
swapUp'_  (Stack t (l:ls) rs) = Stack t ls (l:rs)
swapUp'_  (Stack t []     rs) = Stack t (reverse rs) []

-- | reverse a stack: up becomes down and down becomes up.
reverseStack :: Observable a => Stack a -> Stack a
reverseStack = observe "reverseStack" reverseStack_
reverseStack_ (Stack t ls rs) = Stack t rs ls

--
-- | /O(1) on current window, O(n) in general/. Focus the window 'w',
-- and set its workspace as current.
--
focusWindow :: (Eq s, Eq a, Eq i, Show a,Show i, Show l, Show s, Show sd, Observable a, Observable i, Observable l, Observable s, Observable sd)=> a -> StackSet i l a s sd -> StackSet i l a s sd
...
member :: (Eq a, Show a,Show i, Show l, Show s, Show sd, Observable a, Observable i, Observable l, Observable s, Observable sd) => a -> StackSet i l a s sd -> Bool
...
findTag :: (Eq a, Show a, Show i, Show l, Show s, Show sd, Observable a, Observable i, Observable l, Observable s, Observable sd) => a -> StackSet i l a s sd -> Maybe i
findTag = observe "findTag" findTag_
-- DEFECT
findTag_ a s = case [ tag w | w <- length (workspaces s) `seq` workspaces s, has a (stack w) ] of
    [] -> Nothing
    xs -> Nothing
    where 
    has _ Nothing         = False
    has x (Just (Stack t l r)) = t `seq` (x `elem` (reverse $ l ++ r))
...
--
insertUp :: (Eq a, Show a, Show i, Show l, Show s, Show sd, Observable a, Observable i, Observable l, Observable s, Observable sd) => a -> StackSet i l a s sd -> StackSet i l a s sd
...
--
delete :: (Ord a, Eq s, Show a, Observable a, Observable i, Observable l, Observable s, Observable sd) => a -> StackSet i l a s sd -> StackSet i l a s sd
delete = observe "delete" delete_
delete_ w = sink w . delete' w

-- | Only temporarily remove the window from the stack, thereby not destroying special
-- information saved in the 'Stackset'
delete' :: (Eq a, Eq s, Show a, Observable a, Observable i, Observable l, Observable s, Observable sd) => a -> StackSet i l a s sd -> StackSet i l a s sd
delete' = observe "delete'_" delete'_
delete'_ w s = s { current = removeFromScreen        (current s)
...
swapMaster :: (Show a, Observable a, Observable i, Observable l, Observable s, Observable sd) => StackSet i l a s sd -> StackSet i l a s sd
swapMaster = observe "swapMaster" swapMaster_
swapMaster_ = modify' $ \c -> case c of
...
--
shift :: (Ord a, Eq s, Eq i, Show a, Show i, Show l, Show s, Show sd,Observable a, Observable i, Observable l, Observable s, Observable sd) => i -> StackSet i l a s sd -> StackSet i l a s sd
...
shiftWin :: (Ord a, Eq a, Eq s, Eq i, Show a, Show i, Show l, Show s, Show sd, Observable a, Observable i, Observable l, Observable s, Observable sd) => i -> a -> StackSet i l a s sd -> StackSet i l a s sd
