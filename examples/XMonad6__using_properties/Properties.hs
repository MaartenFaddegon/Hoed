...
-- subjFun = insertUp
prop_insertUp_I subjFun n (x :: T) = invariant $ subjFun n x
...
-- MF added this specification myself
spec_member subjFun w (x :: T) = (w `elem` allWindows_noNub x) == subjFun w x

allWindows_noNub :: Eq a => StackSet i l a s sd -> [a]
allWindows_noNub = concatMap (maybe [] integrateL . stack) . workspaces
 where
 concatMapL f = concatL . map f
 concatL = foldl (++) []
 integrateL (Stack x l r) = x : l ++ r
