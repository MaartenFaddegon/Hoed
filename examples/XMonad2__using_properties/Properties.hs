...
prop_swap_master_I (x :: T) = invariant x ==> invariant $ swapMaster x

prop_swap_left_I  (n :: NonNegative Int) (x :: T) =
    invariant x ==> invariant $ foldr (const swapUp ) x [1..n]
prop_swap_right_I (n :: NonNegative Int) (x :: T) =
    invariant x ==> invariant $ foldr (const swapDown) x [1..n]
