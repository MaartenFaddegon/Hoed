...
evalProposition unevalGen trc v ms prop = do
  ...
  case exit' of 
    ...
    generateCode = do -- Uncomment the next line to dump generated program on screen
                      ...
                      return prgm
                      where prgm :: String
                            prgm = (generate unevalGen prop ms trc getEvent i f)
...
generate :: PropVarGen String -> Proposition -> [Module] -> Trace -> (UID->Event) -> UID -> String -> String
generate unevalGen prop ms trc getEvent i f = generateHeading prop ms ++ generateMain unevalGen prop trc getEvent i f
...
generateHeading prop ms =
  ...
  ++ generateImport (Module "Debug.Hoed.Pure" [])
  ++ qcImports
...
generateMain :: (PropVarGen String) -> Proposition -> Trace -> (UID->Event) -> UID -> String -> String
generateMain unevalGen prop trc getEvent i f
  = "main = runOstore $ " ++ propVarBind (foldl accArg ((propName prop)++ " ",propVars0) (argMap prop)) prop ++ "\n" -- was reverse . argMap $ prop
    where 
    ...
    cf = foldl1 (liftPV $ \acc c -> acc ++ " " ++ c)
           [ propVarReturn $ "(conAp " ++ f
...
generateRes' unevalGen trc getEvent (Just e)
  | change e == Fun = case dfsChildren frt e of [a] -> generateRes' unevalGen trc getEvent a
